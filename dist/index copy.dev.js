...
// So executa a callback quando a pagina terminar de carregar
window.onload = function () {
  // Adiciona handler do evento click no botao de Start Game
  document.getElementById("start-button").onclick = function () {
    // Esconder o titulo e o botao de Start Game
    document.getElementById("title").style.display = "none"; // Exibir a game board

    document.getElementById("game-board").style.display = "block"; // Iniciar o jogo
  ...
  function startGame() {
    // Iniciar o jogo
    myGameArea.start(); // Carrega imagem de fundo do canvas

    background = new Background("./images/bg.png"); // Instancia um novo objeto para o jogador

    player = new Component(80, 50, "./images/flappy.png", 100, 110); // Inicializar uma array vazia de obstaculos
  ...
  var myGameArea = {
    ...
    drawCanvas: function drawCanvas() {
      ...
      this.canvas.height = 500; // Extrai o context 2D do canvas

      this.context = this.canvas.getContext("2d"); // Adicionar o canvas dentro da div de id "game-board"

      document.getElementById("game-board").append(this.canvas);
    },
    start: function start() {
      this.drawCanvas(); // Antes do final de cada repaint, execute a funcao updateGameArea. Isso gera um novo repaint, criando um ciclo infinito
  ...
  function Background(source) {
    ...
    this.img.onload = function () {
      ...
      that.imgH = that.img.height * that.scale; // if (that.imgW > myGameArea.canvas.width) {
  ...
  function Component(width, height, image, x, y) {
    ...
    this.crashWith = function (obstacle) {
      ...
      return !(player.bottom() < obstacle.top() || player.top() > obstacle.bottom() || player.right() < obstacle.left() || player.left() > obstacle.right());
  ...
  function createObstacle() {
    ...
    x = myGameArea.canvas.width; // Pega a altura do canvas

    y = myGameArea.canvas.height; // Gera um valor aleatorio de altura pro obstaculo

    height = Math.floor(Math.random() * (200 - 20 + 1) + 20); // Gera um valor aleatorio de espacamente entre diferentes obstaculos

    gap = Math.floor(Math.random() * (200 - 100 + 1) + 100); // Gera obstaculos partindo do lado de cima do canvas

    myGameArea.myObstacles.push(new Component(70, height, "./images/obstacle_top.png", x, 0)); // Gera obstaculos partindo do lado de baixo do canvas
  ...
  function updateGameArea() {
    ...
    myGameArea.clear();
    background.draw(); // Atualiza a posicao no eixo X dos obstaculos pra eles "andarem" pra esquerda
    ...
    player.newPos();
    player.update();
    myGameArea.score(); // Se o jogador cair no "chao" ou voar pra cima do "teto", game over
